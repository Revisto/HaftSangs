...
func _ready():
	...
	if Globals.network_manager:
		Globals.network_manager.connect("slingshot_updated", self, "_on_slingshot_updated")
	Globals.game_manager.connect("launch_projectile", self, "_on_launch_projectile")
...
func send_slingshot_update():
	if Globals.network_manager:
		Globals.network_manager.send_slingshot_update(elastic_control_point.global_position, state, launch_impulse)
	else:
		print("Network manager is null, cannot send update")

func _on_slingshot_updated(slingshot_data):
	print("Slingshot updated: ", slingshot_data)
	var position_data = slingshot_data["position"]
	var position = Vector2(position_data[0], position_data[1])
	var income_state = slingshot_data["state"]
	var launch_impulse_data = slingshot_data["launch_impulse"]
	var launch_impulse = Vector2(launch_impulse_data[0], launch_impulse_data[1])
	
	if projectile:
		projectile.global_position = position
	if elastic_control_point:
		elastic_control_point.global_position = position
...
func _on_lock_controls(lock_move_obstacles, lock_shooting):
	locked = lock_shooting
	$InputArea.locked = locked
...
func _on_launch_projectile():
	 
	var small_impulse = Vector2(2000, -2000)
	launch(small_impulse)
	trajectory_drawer.clear()
	var t = Tween.new()
	add_child(t)
	t.interpolate_property(
